...
import { CaddyRouteProjectAssociation } from "@/components/CaddyRouteProjectAssociation";
...
export function CaddySettingsEditor() {
  ...
  const [routeLinks, setRouteLinks] = useState<
    Record<string, RouteProjectLink[]>
  >({});
  ...
  const fetchConfig = useCallback(async () => {
    ...
    setError("");
    try {
      const [configRes, routingRes] = await Promise.all([
        fetch("/api/caddy/config"),
        fetch("/api/settings/project-routing"),
      ]);
      const data = (await configRes.json()) as CaddyConfigResponse & {
        error?: string;
      };
      if (!configRes.ok) {
      ...
      if (routingRes.ok) {
        const routing = (await routingRes.json()) as {
          ...
          routeLinks?: Record<string, RouteProjectLink[]>;
        };
        setProjects(
          (routing.projects ?? []).map((project) => ({
            id: project.id,
            name: project.name,
          })),
        );
        setRouteLinks(routing.routeLinks ?? {});
      }
    } catch {
  ...
  async function updateRouteProjectLink(
    routeKey: string,
    projectId: string,
    linked: boolean,
  ): Promise<boolean> {
    try {
      const res = await fetch("/api/settings/project-routing/link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ projectId, routeKey, linked }),
      });
      const body = (await res.json()) as { error?: string };
      if (!res.ok) {
        setError(body.error ?? "Failed to update project link");
        return false;
      }
      await fetchConfig();
      return true;
    } catch {
      setError("Network error while updating project link");
      return false;
    }
  }
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500">
                ...
                Associate routes with projects here or from Project routing /
                each project&apos;s Config &amp; history tab.
              </p>
            </div>
            <button
              type="button"
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0 space-y-2">
                      ...
                      <CaddyRouteProjectAssociation
                        routeKey={route.key}
                        linkedProjects={routeLinks[route.key] ?? []}
                        projects={projects}
                        disabled={saving}
                        onLinkChange={(projectId, linked) =>
                          updateRouteProjectLink(route.key, projectId, linked)
                        }
                      />
                    </div>
