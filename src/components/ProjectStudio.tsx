...
function projectPollIntervalMs(
  ...
  if (mode === "agents" || mode === "changes") return 12_000;
  if (mode === "settings" && !isDeploying) return 12_000;
...
export function ProjectStudio({ mode }: { mode: ProjectMode }) {
  ...
  useEffect(() => {
    ...
    const target = legacyProjectSearchToPath(
      id,
  ...
  function openAgentSession(sessionId: string) {
    router.replace(projectModeHref(id, "agents", { session: sessionId }), {
      scroll: false,
    });
  }
  ...
  function selectTab(tab: ProjectMode) {
    if (tab === "agents") {
      const session = searchParams.get("session");
      router.replace(
        projectModeHref(id, "agents", session ? { session } : undefined),
        { scroll: false },
      );
      return;
    }
    if (tab === "changes") {
      const keep = ["mode", "base", "head", "branch", "source", "onto", "target", "file", "session"];
      const query: Record<string, string> = {};
      for (const key of keep) {
        const value = searchParams.get(key);
        if (value) query[key] = value;
      }
      router.replace(projectModeHref(id, "changes", query), { scroll: false });
      return;
    }
    router.replace(projectModeHref(id, tab), { scroll: false });
  }
      ...
      {activeTab === "overview" ? (
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain space-y-5">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <Button variant="secondary" onClick={() => selectTab("changes")}>
              Changes
            </Button>
        ...
        >
          <div className={deployBusy ? "shrink-0 space-y-4 overflow-y-auto max-h-[40%]" : undefined}>
          {forgefileStatus && forgefileStatus.status !== "valid" ? (
            <div className="mb-6">
              <ForgefileStatusBanner
                projectId={project.id}
          ...
          <div className={deployBusy ? "min-h-0 shrink overflow-y-auto" : undefined}>
